...
function isTeamMatch(uiName, apiName) {
  ...
  return similarity(ui, api) > 0.6;
}

// ─── Flags ───────────────────────────────────────────────────────────────────
const USE_MOCK      = true;
const DRY_RUN       = false;
const STAKE_AMOUNT  = 1; // KES — fixed stake per leg, always
...
(async () => {
  ...
  console.log(`  STAKE_AMOUNT: KES ${STAKE_AMOUNT} per leg`);
  console.log(`  USE_MOCK    : ${USE_MOCK}`);
  console.log(`  PROFILE_DIR : ${PROFILE_DIR}`);
  console.log('─'.repeat(55));

  if (!DRY_RUN) {
    console.log(`🔴 LIVE MODE: will stake KES ${STAKE_AMOUNT} per leg with real money!`);
  }
...

  {
...

    {
      ...
      // ✅ Bulk-extract ALL team name pairs in ONE page.evaluate call.
      //    Old approach: 2 × textContent() IPC calls per match × 75 matches = 150 round-trips.
      //    New approach: 1 round-trip total, JS runs inside the browser process.
      const awaySelector = resolvedHomeSelector.replace('home', 'away');
      const t_extract = timer('Name extraction');
      const allMatchNames = await page.evaluate(
        ({ containerSel, homeSel, awaySel }) =>
          Array.from(document.querySelectorAll(containerSel)).map(el => ({
            home: (el.querySelectorAll(`${homeSel} span`)[1]
                ?? el.querySelector(`${homeSel} span`))?.textContent?.trim() ?? '',
            away: (el.querySelectorAll(`${awaySel} span`)[1]
                ?? el.querySelector(`${awaySel} span`))?.textContent?.trim() ?? '',
          })),
        { containerSel: resolvedContainerSelector, homeSel: resolvedHomeSelector, awaySel: awaySelector }
      );
      record('Name extraction', t_extract.end());
      console.log(`📋 Extracted ${allMatchNames.length} match name pairs in one shot`);

      let clickedCount = 0;
      const betTimings = [];

      for (let i = 0; i < allMatchNames.length; i++) {
        const { home, away } = allMatchNames[i];
        ...
        try {
          ...
          for (const opp of opportunities) {
            ...
            if (homeMatch && awayMatch) {
              ...
              const homeStake = resolveStake();
              const awayStake = resolveStake();
      ...
      record('Match loop', Date.now() - RUN_START - Object.values(timings).reduce((a,b)=>a+b,0));
      ...
      const phases = [
        ...
        ['Name extraction', timings['Name extraction']],
      ];
